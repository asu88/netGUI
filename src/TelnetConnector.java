...
//Hilo de ejecuci�n que crea un canal TCP para comunicarse con la m�quina virtual a trav�s
//del protocolo Telnet.
    public class ConnectorThread extends Thread {
        ...
        @Override
        public void run() {
            String errorMsg = "No se ha podido arrancar la m�quina " + node.getName() + ".\n"
                    + "A continuaci�n se reiniciar� su ejecuci�n.";
            int MAXATT = 100, i = 1;
            boolean connected = false;
            try {
//                System.out.println("Se ha arrancado el hilo de conector");
                sleep(3000);
                while ((!connected) && (i <= MAXATT)) {
                    // System.out.println(+ node.getName() + " intentando conectar");
//                    System.out.println("Tratando de hacer openTelnetConnection");
                    connected = openTelnetConnection();
//                    System.out.println("OpenTelnetConnection?? -> " + connected);
                    sleep(500);
                    i++;
                }
                if (connected) {
                    System.out.println(node.getName() + " connected to GUI");
                    NetKitViewer.log.addText("> " + node.getName() + " connected to GUI");
                    //Preparamos el socket para leer desde el inductor de la shell
                    //NodeTelnetCommunicator2.getTelnetSocket(node).read();
                    node.setStarted(true);
                } else {
                    ...
                    //Esperamos a que la m�quina virtual finalice de manera ordenada
                    sleep(18000);
                    node.startNetKit();
                }
