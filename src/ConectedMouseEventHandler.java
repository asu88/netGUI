...
public class ConectedMouseEventHandler extends PBasicInputEventHandler
{
	...
	private void processClick (PInputEvent aEvent)
	{
		if (aEvent.getPickedNode() instanceof NKNode )
			if (!iconClicked)
				...
				//segundo click (creamos una conexion) 
	...
	private void processRoutersConnectSituation (PInputEvent aEvent, boolean click)
	{
		...
		else processConnectNormalSituation (aEvent, click);
	}

	private void processCurrentSituation (PInputEvent aEvent, boolean click)
	{
		if (!node1.equals((NKNode)aEvent.getPickedNode()))
			//El nodo seleccionado es distinto del que se ha "pinchado" anteriormente
			if (node1 instanceof NKCompaq)
			    	//Es un terminal, en tal caso s�o se permite su conexion a un hub y switch
				processConnectNormalSituation(aEvent, click);
			if (node1 instanceof NKRouter)
				//Los routers pueden conectarse tanto a hubs como a otros routers y switch
